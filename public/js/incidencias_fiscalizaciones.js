(function($) {
    $(document).ready(function() {
        ...
        let $createCedula = $('#create_cedula');
        try {
            if (typeof $.fn.select2 !== 'undefined' && $createCedula.length) {
                $('#createModal').on('shown.bs.modal', function() {
                    $createCedula.select2({
                        ...
                        ajax: {
                            ...
                            processResults: function(data) {
                                if (data.error) {
                                    console.error('Error loading active workers:', data.details);
                        ...
                        if (data) {
                            $createCedula.val(data.id).trigger('change'); // Sincronizar el valor del input
                            $('#create_trabajador').val(data.trabajador || '');
                            $('#create_ubicacion').val(data.ubicacion || '');
                            $('#create_contacto').val(data.contacto || '');
                            $createCedula.next('.select2-container').find('.select2-selection__rendered').text(data.text); // Actualizar visualmente
                        }
                    }).on('select2:unselect', function() {
                        $createCedula.val(null).trigger('change');
                        $('#create_trabajador').val('');
                        $('#create_ubicacion').val('');
                        $('#create_contacto').val('');
                        $createCedula.next('.select2-container').find('.select2-selection__rendered').text('');
                    });
                });
                $('#createModal').on('hidden.bs.modal', function() {
                    $createCedula.val(null).trigger('change').select2('destroy');
                    $('#create_trabajador').val('');
                    $('#create_ubicacion').val('');
                    $('#create_contacto').val('');
                });
            } else {
                console.warn('Select2 library not loaded or #create_cedula not found.');
                Swal.fire({
                    icon: 'warning',
                    title: 'Advertencia',
                    text: 'Select2 no cargado o el campo #create_cedula no se encuentra.',
                    confirmButtonText: 'Aceptar',
                    customClass: { confirmButton: 'btn btn-primary' },
                });
            }
        ...
        // Fetch fiscalizacion data by cedula
        function fetchFiscalizacionByCedula(cedula, prefix, callback) {
            $.ajax({
                ...
                error: function(xhr) {
                    console.error('Error fetching fiscalizacion:', xhr.responseText);
                    $(`#${prefix}_trabajador`).val('');
                    $(`#${prefix}_ubicacion`).val('');
                    $(`#${prefix}_contacto`).val('');
                    callback(false);
        ...
        function initializeDataTable() {
            if (!$('#incidenciasFiscalizacionesTable').length || !$('#incidenciasFiscalizacionesTable').is('table')) {
                console.error('Table #incidenciasFiscalizacionesTable not found or is not a table element');
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'La tabla #incidenciasFiscalizacionesTable no se encuentra en la página o no es una tabla válida.',
                    confirmButtonText: 'Aceptar',
                    customClass: { confirmButton: 'btn btn-primary' },
                });
                return;
            ...
            try {
                var table = $('#incidenciasFiscalizacionesTable').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: {
                    ...
                    columns: [
                        { data: 'cedula', title: 'Cédula', responsivePriority: 1 },
                ...
                $('#incidenciasFiscalizacionesTable tbody').on('click', 'tr', function() {
                ...
                $('#submitCreate').on('click', function() {
                    ...
                    console.log('Creating with form data:', formData);
                    $.ajax({
                ...
                $('#incidenciasFiscalizacionesTable').on('click', '.delete-incidencia', function() {
                    var id = $(this).data('id');
                    Swal.fire({
                        icon: 'warning',
        ...
        function validateForm(formId, isEdit = false) {
            ...
            requiredFields.forEach(field => {
                ...
                let valueToCheck = $input.val().trim();
                // Caso especial para cedula con Select2
                if (field === `${prefix}cedula` && $createCedula.length && $createCedula.hasClass('select2-hidden-accessible')) {
                    const select2Data = $createCedula.select2('data');
                    if (select2Data.length > 0) {
                        valueToCheck = select2Data[0].id;
                        $input.val(valueToCheck); // Sincronizar el input con el valor de Select2
                    }
                }
